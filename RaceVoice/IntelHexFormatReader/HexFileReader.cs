...
namespace IntelHexFormatReader
{
    public class HexFileReader
    {
        ...
        private static MemoryBlock ReadHexFile(IEnumerable<string> hexRecordLines, int memorySize)
        {
            ...
            int picbase = 0x1d000000; // 0x1d00:0000
                                      // int picbase = 0x1fc00000; // boot flash
                    ...
                    case RecordType.Data:
                        {
                            ...
                            //globals.WriteLine("Addr=" + nextAddress);
                            // adjust so we fall within a 0 to 512KB address space ... not in the 4GB range of the PIC32
                            if (nextAddress >= picbase && nextAddress <= (picbase + (256 * 1024)))
                            {
                                nextAddress -= picbase;
