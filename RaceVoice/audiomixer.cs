...
namespace RaceVoice
{
    ...
    {
        public static int GetProcessID(string shortname)
        {
            string fullname = "";
            string app = "";
            int pid = 0;
            ...
            foreach (Process process in processlist)
            {
                ...
                {
                    app = process.ProcessName.ToLower();
                    Console.WriteLine("Process:   {0}", app);
                    Console.WriteLine("    ID   : {0}", process.Id);
                    Console.WriteLine("    Title: {0} \n", process.MainWindowTitle);
                }

                if (app.Contains(shortname))
                {
                    pid = process.Id;
                    fullname = app;
                    break;
            ...
            try
            {
                ...
                for (int i = 0; i < count; ++i)
                {
                    ...
                    try
                    {
                        ...
                        string nn;
                        ctl.GetDisplayName(out nn);
                        // NOTE: we could also use the app name from ctl.GetDisplayName()
                        int cpid;
                        ctl.GetProcessId(out cpid);

                        globals.WriteLine("CTL=" +cpid);
                        if (cpid == pid)
                        {
